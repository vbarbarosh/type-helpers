...
const SP = Symbol('empty_value for safe_int');
...
describe('safe_int', function () {
    ...
    describe('should handle edge values', function () {
        edge_values.forEach(function (item) {
            it(item.label, function () {
                ...
                case '-0.50':
                case '-0.51':
                    assert.strictEqual(safe_int(item.value, SP), 0);
                    break;
                case 'true':
                    assert.strictEqual(safe_int(item.value, SP), 1);
                    break;
                case '1e100':
                case '10n**100n':
                case 'Infinity':
                ...
                case 'Number.POSITIVE_INFINITY':
                    assert.strictEqual(safe_int(item.value, SP), Number.MAX_SAFE_INTEGER);
                    break;
                case '-(10n**100n)':
                case '-Infinity':
                case 'Number.MIN_SAFE_INTEGER':
                case 'Number.NEGATIVE_INFINITY':
                    assert.strictEqual(safe_int(item.value, SP), Number.MIN_SAFE_INTEGER);
                    break;
                default:
                    assert.strictEqual(safe_int(item.value, SP), SP);
                    break;
                }
            });
        });
    ...
    describe('basic usage', function () {
        ...
        it('should respect input value when default value was set', function () {
            ...
            assert.strictEqual(safe_int(0.55, SP), 0);
            assert.strictEqual(safe_int(-0, SP), 0);
            assert.strictEqual(safe_int(-0.1, SP), 0);
            assert.strictEqual(safe_int(-0.5, SP), 0);
            assert.strictEqual(safe_int(-0.51, SP), 0);
            assert.strictEqual(safe_int(-0.55, SP), 0);
            assert.strictEqual(safe_int(Number.MIN_VALUE, SP), 0);
            assert.strictEqual(safe_int(Number.MIN_SAFE_INTEGER, SP), Number.MIN_SAFE_INTEGER);
        });
    });
});
